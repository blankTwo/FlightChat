...
(() => {
  ...
  const installCookieImportButton = () => {
    ...
    button.addEventListener('click', () => {
      ...
      Object.assign(mask.style, { position: 'fixed', inset: '0', zIndex: '2147483647', display: 'grid', placeItems: 'center', padding: '20px', boxSizing: 'border-box', overflowY: 'auto', background: 'rgba(38, 24, 14, .34)' });
      const panel = document.createElement('form');
      Object.assign(panel.style, { display: 'flex', flexDirection: 'column', width: 'min(560px, calc(100vw - 40px))', maxHeight: 'calc(100dvh - 40px)', boxSizing: 'border-box', padding: '22px', overflow: 'hidden', background: '#fffaf3', borderRadius: '14px', boxShadow: '0 18px 52px rgba(0,0,0,.28)', color: '#382417', font: '14px system-ui, sans-serif' });
      panel.innerHTML = '<strong style="font-size:17px;flex:0 0 auto">导入 Cookie</strong><p style="flex:0 0 auto;margin:8px 0 14px;color:#6a5648">粘贴 Cookie-Editor 导出的 JSON 数组，仅接受 chatgpt.com / openai.com Cookie。</p><textarea autofocus placeholder="[ { &quot;name&quot;: &quot;…&quot;, &quot;value&quot;: &quot;…&quot;, &quot;domain&quot;: &quot;.chatgpt.com&quot; } ]" style="display:block;box-sizing:border-box;flex:1 1 190px;min-height:104px;width:100%;height:auto;resize:vertical;padding:10px;border:1px solid #d4c4b3;border-radius:8px;font:12px ui-monospace,monospace"></textarea><div data-actions style="display:flex;flex:0 0 auto;justify-content:flex-end;gap:10px;margin-top:14px"><button id="flight-cookie-cancel" type="button" data-cancel style="border:0;background:transparent;padding:8px 12px;cursor:pointer">取消</button><button id="flight-cookie-submit" type="submit" style="border:0;background:#5a351e;color:#fffaf3;border-radius:7px;padding:9px 14px;cursor:pointer">导入并刷新</button></div><p data-status style="flex:0 0 auto;min-height:18px;margin:10px 0 0;color:#a13e31"></p>';
